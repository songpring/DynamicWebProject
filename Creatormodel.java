...
class Creator_model extends CI_Model {
	...
	public function do_listData($data) {
		...
		if(!empty($data['search'])) {
			$this->db->group_start();
			$array = array('phonenumber' => $data['search'], 'name' => $data['search'], 'nationality' => $data['search'], 'address' => $data['search'], 'email' => $data['search'], 'sns' => $data['search']);
		...
		// 최고 관리자가 아님 조건절 포함.
		if($user_data['level'] < 9) $this->db->where('user_id', $user_data['user_id']);
		...
		$this->db->select(' id, cate, name, nationality, birthday, gender, address, phonenumber, email, sns, reg_datetime ');
	...
	public function do_getData($data) {
