...
class Contactus_model extends CI_Model {
	...
	public function do_listData($data) {
		...
		if(!empty($data['search'])) {
			$this->db->group_start();
			$array = array('name' => $data['search'], 'nationality' => $data['search'], 'phonenumber' => $data['search'], 'sns' => $data['search'],'instagram' => $data['search'], 'uploadfiles' => $data['search'], 'extends_var' => $data['search']);
		...
		$this->db->select(' id, cate, name, nationality, phonenumber, birthday, sns, instagram, uploadfiles, extends_var, reg_datetime ');
